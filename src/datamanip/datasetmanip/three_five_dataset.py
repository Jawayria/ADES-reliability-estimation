...
class ThreeFiveDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        if self.match == 'multiclass':
            return ["multiclass_model_train_data.pt", "multiclass_model_test_data.pt", "multiclass_model_val_data.pt"]
        else:
            return ['1-1_model_train_data.pt', '1-1_model_test_data.pt', '1-1_model_val_data.pt',
                    '2-1_model_train_data.pt', '2-1_model_test_data.pt', '2-1_model_val_data.pt',
                    '2-2_model_train_data.pt', '2-2_model_test_data.pt', '2-2_model_val_data.pt',
                    '3-1_model_train_data.pt', '3-1_model_test_data.pt', '3-1_model_val_data.pt',
                    '3-2_model_train_data.pt', '3-2_model_test_data.pt', '3-2_model_val_data.pt',
                    '3-3_model_train_data.pt', '3-3_model_test_data.pt', '3-3_model_val_data.pt',
                    '3-4_model_train_data.pt', '3-4_model_test_data.pt', '3-4_model_val_data.pt',
                    'ensemble_train_data.pt', 'ensemble_test_data.pt', 'ensemble_val_data.pt']
    ...
    def process(self):
        ...
        if self.match == 'multiclass':
            full_range = (0, 4, 8)
            converted_range = convert_range_to_floats(full_range)

            for j, df in enumerate(sets):  # for train/test/val
                ...
                multiclass_rels_tensor = construct_reliability_classes(trimmed_df, converted_range[1], binary=False)

                multiclass_data_list = self.compose_data_list(all_edge_indices, multiclass_rels_tensor,
                                                            node_features_tensor)

                # Save to the corresponding ensemble processed paths
                self.save(multiclass_data_list, self.processed_paths[-(3 - j)])
                print(f"Finished processing multiclass dataset: {['train', 'test', 'val'][j]}")
